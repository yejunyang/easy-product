...
import ai.yunxi.core.domain.entity.TbBase;
import ai.yunxi.core.domain.entity.TbOrderItem;
import ai.yunxi.core.domain.entity.TbShiroFilter;
import ai.yunxi.core.domain.entity.TbUser;
...
public class SystemController {
    ...
    public DataTablesResult getLog(int draw, int start, int length,
                                   @RequestParam("search[value]") String search,
                                   @RequestParam("order[0][column]") int orderCol,
                                   @RequestParam("order[0][dir]") String orderDir) {
        //获取客户端需要排序的列
        String[] cols = {"checkbox", "id", "name", "type", "url", "request_type",
                "request_param", "user", "ip", "ip_info", "time", "create_date"};
        ...
        DataTablesResult result = systemService.getLogList(draw, start, length, search,
